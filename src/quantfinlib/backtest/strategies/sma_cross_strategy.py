"""Golden/death cross strategy (port of Java
``backtest.strategies.SmaCrossStrategy``)."""
...
class SmaCrossStrategy(TradingStrategy):
    ...
    def name(self) -> str:
        return f"SMA_CROSS({self._fast_period},{self._slow_period})"

    def init(self, series: BarSeries) -> None:
        self._fast = Indicators.sma(series.closes(), self._fast_period)
        self._slow = Indicators.sma(series.closes(), self._slow_period)
